...
def draw_graticule(
    ...
    # If a label is within 5% of the perpendicular edge, skip it.
    x_margin = width * 0.05
    ...
    for lon in lons:
        ...
        if len(valid_indices) > 0:
            ...
            # Check 1: Corner proximity (Longitude label is close to left or right border?)
            if x_pos < minx + x_margin or x_pos > maxx - x_margin:
                continue
                
            # Check 2: Exclusion zones (e.g., Inset map)
            if exclusion_zones:
                p = Point(x_pos, y_pos)
    ...
    for lat in lats:
        ...
        if len(valid_indices) > 0:
            ...
            # Check 1: Corner proximity (Latitude label is close to top or bottom border?)
...
def create_main_map(
    ...
    exclusion_zones : list
        Polygons where graticule text should not be drawn (e.g. inset map location)
...
def create_inset_map(
    ...
    ax_ins = inset_axes(
        ...
        loc='upper left', 
        bbox_to_anchor=(0.04, 0, 1, 1), # Shifted 4% to the right
    ...
    # Add Lat/Lon Graticule with corner exclusion
    # Coarser step (4 degrees) for the large map
    draw_graticule(ax_ins, (minx - pad_x, miny - pad_y, maxx + pad_x, maxy + pad_y), 
                   TARGET_CRS, lon_step=4.0, lat_step=4.0, fontsize=6, color='#999999',
                   zorder=3)
...
def main() -> None:
    ...
    # Calculate inset box for river label placement AND graticule exclusion
    # The inset is anchored at 4% of width from the left.
    view_minx, view_miny, view_maxx, view_maxy = gdf_proj.total_bounds
    view_w = (view_maxx + BUFFER_METERS) - (view_minx - BUFFER_METERS)
    view_h = (view_maxy + BUFFER_METERS) - (view_miny - BUFFER_METERS)
    
    # Inset is 30% of main map dimensions, offset by 4% from left
    inset_w = view_w * INSET_SIZE_FRACTION
    inset_h = view_h * INSET_SIZE_FRACTION
    inset_x = (view_minx - BUFFER_METERS) + view_w * 0.04
    # Inset is top-aligned
    inset_y_top = (view_maxy + BUFFER_METERS)
    inset_y_bottom = inset_y_top - inset_h
    
    inset_box = box(inset_x, inset_y_bottom, inset_x + inset_w, inset_y_top)
    
    # Create main map (Pass inset_box as exclusion zone for graticule)
    fig, ax = create_main_map(
        gdf_proj, lakes_final, rivers_final, ocean_final,
        water_polygons, gdf_proj.total_bounds,
        exclusion_zones=[inset_box]
    )
